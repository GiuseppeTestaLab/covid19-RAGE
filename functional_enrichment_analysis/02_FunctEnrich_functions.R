...
library(heatmap3)
library(STRINGdb); string_db=STRINGdb$new( version="11", species=9606, score_threshold=0, input_directory="")


####_____----
## mat.idx----
# _ given a data matrix and a vector of matrix indices, returns a matrix with the original matrix indices and the corresponding row and column coordinates for each index)----
mat.idx <- function(mat = datamat, ## matrix to select elements from
                    idx = idx.vector ## vector with indices of elements to be selected
) {
  cn1 <- cn0 <- idx / nrow(mat)
  idx.cn1 <- which(cn1 != trunc(cn0))
  if (length(idx.cn1) > 0) cn1[idx.cn1] <- (trunc(cn0[idx.cn1]) + 1)
  rw1 <- round(((cn0 + 1) - cn1) * nrow(mat), 0)
  idx2 <- cbind(idx, rw1, cn1, mat[idx])
  colnames(idx2) <- c("idx", "row", "col", "value")
...
####_____----
## hheatmap2----
# _ given a data matrix and the related metadata plots a customized Heatmap----
